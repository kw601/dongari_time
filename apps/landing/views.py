...
from apps.community.models import Club, Board
...
def signup(request):
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            messages.error(
                request, "회원가입 중 오류가 발생했습니다. 입력 정보를 확인하세요."
            )
    else:
        form = SignupForm()
            ...
            for field, errors in form.errors.items():
                for error in errors:
                    if field == "old_password" and "incorrect" in error.lower():
                        ...
                        messages.error(
                            request, "새 비밀번호와 비밀번호 확인이 일치하지 않습니다."
                        )
                    else:
        ...
        if request.method == "POST":
            ...
            try:
                ...
                if code == club.club_num:  # 코드가 일치하면
                    # 중복 가입 방지
                    if Auth_Club.objects.filter(
                        user_id=request.user, club_id=club
                    ).exists():
                        messages.error(request, "이미 가입된 동아리입니다.")
                        return render(
                            request,
                            "landing/club_auth.html",
                            {"error": "이미 가입된 동아리입니다."},
                        )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            try:
                ...
                return redirect(
                    reverse("landing:find_username_result") + f"?username={username}"
                )
            except User.DoesNotExist:
                form.add_error(None, "해당 이름과 이메일로 사용자를 찾을 수 없습니다.")
            except User.MultipleObjectsReturned:
                form.add_error(
                    None, "해당 이름과 이메일로 여러 사용자가 검색되었습니다."
                )
    else:
